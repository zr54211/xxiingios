...
//   Методы:
//     НачатьСканирование / StartScanning(НастройкиJSON)  - открыть экран сканера (этап 2/3)
...
//   Событие (через ВнешнееСобытие):
//     Источник "BarcodeScannerZXing", Событие "ScanResult", Данные - JSON результата
...
public:
	...
	// Отправка результата сканирования в 1С (ВнешнееСобытие).
	// Вызывается платформенными слоями (android/, ios/) из потока камеры.
