...
Статус: **все этапы выполнены** (июль 2026), документ сохранён как обоснование
принятых решений; актуальное состояние — `ios/README.md`.

Цель: паритет с Android-частью. Тот же контракт Native API (НачатьСканирование /
ЗавершитьСканирование / РаспознатьКадр / ВключитьФонарик,
...
0. **Прототип риска UI** (главное): из метода ВК показать/скрыть пустой
   ...
   Итог: прототип взлетел, режим Б не потребовался — метод удалён из контракта.
